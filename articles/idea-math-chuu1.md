...
- 特定の値を整数の平方にするためには、素因数分解を用いて乗算する素数を決定する
  ＊例えば2^6と8^2は同じであるから、特定値が2^5なら2を乗算する
...
- 関係式は関数y=axの形のようにyを先頭に書かずxが先でも良い
...
- 比例：xとyの関係がy=axとなるとき 「yはxに比例する」という
- 反比例：xとyの関係がxy=aとなるとき 「yはxに反比例する」という
- 比例定数：上記aのこと
